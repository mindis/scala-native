...
object stdio {
  ...
  def setvbuf(stream: Ptr[FILE], buffer: Ptr[CChar], mode: CInt, size: CSize): CInt = extern
  def fwide(stream: Ptr[FILE], mode: CInt): CInt = extern

  // Direct input/output

  def fread(buffer: Ptr[_], size: CSize, count: CSize, stream: Ptr[FILE]): CSize = extern
  def fwrite(buffer: Ptr[_], size: CSize, count: CSize, stream: Ptr[FILE]): CSize = extern
  ...
  def snprintf(buffer: CString, bufsz: CInt, format: CString, args: Vararg*): CInt = extern
  ...
  @struct class fpos_t private (private val data: Long)

  // Macros

  @name("scalanative_libc_stdin")
  def stdin: Ptr[_] = extern
